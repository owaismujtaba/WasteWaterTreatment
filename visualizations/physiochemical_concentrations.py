...
from utils import get_significance_mark, calculate_reduction_rate_physiochemical
import pdb
...
def plot_physiochemical_reduction_percentage(nrows=3, nclos=4):
    path = config.PHYSIOCHEMICL_PARAMETERS_PATH
    table1_percentage_diff = calculate_reduction_rate_physiochemical(path)
    data = table1_percentage_diff
    subplot_titles = ['a)', 'b)', 'c)', 'd)', 'e)', 'f)', 'g)', 'h)', 'i)', 'j)', 'k)']

    title_index = 0

    df = pd.DataFrame(data).set_index("Parameters")
    x_labels = ["RawSewage", "1:1", "1:3", "1:7"]
    x_pos_full = np.arange(len(x_labels))
    x_pos = np.arange(len(x_labels) - 1)

    fig, axes = plt.subplots(nrows, nclos, figsize=(15, 10), sharey=False)
    axes = axes.flatten()
    for i, metal in enumerate(df.index):
        ...
        dil_eq = f"y = {slope_dil:.2f}x + {intercept_dil:.2f}"
        axes[i].text(0.05, 0.75, dil_eq, color='b', fontsize=12, rotation=0, transform=axes[i].transAxes, ha='left', va='top')
        ...
        ctrl_eq = f"y = {slope_ctrl:.2f}x + {intercept_ctrl:.2f}"
        axes[i].text(0.05, 0.3, ctrl_eq, color='g', fontsize=12, rotation=0, transform=axes[i].transAxes, ha='left', va='top')
        axes[i].text(-0.05, 1.1, subplot_titles[title_index], transform=axes[i].transAxes, 
        ...
        title_index +=1
        if i ==0 :
            axes[i].set_xlabel('Dilution ratios')
            axes[i].set_ylabel('Concentration decrease %')

    legend_colors = ['b', 'g']
    condition_names = ['control', 'dilution']
    handles = [mpatches.Patch(color=color, label=label) for color, label in zip(legend_colors, condition_names)]
    axes[11].legend(handles=handles, loc='upper left', bbox_to_anchor=(0.3, 0.75), ncol=1)
    ...
    plt.tight_layout()
    plt.savefig('Images/physiochemical_parameters_percentage_decrease.png', dpi=config.DPI)
    

def plot_physiochemical_distributions(nrows=4, ncols=3):
    table1 = pd.read_csv(config.PHYSIOCHEMICL_PARAMETERS_PATH)
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            sns.boxplot(data=plot_data, x='Condition', y='Concentration', ax=axes[row, col], 
            ...
            t_statistic, p_value = calculate_t_static_and_p_values(
                plot_data=plot_data,
                conditions=['RawSewage', '1:1_Dilution', '1:1_Control', '1:3_Dilution', 
                            '1:3_Control', '1:7_Dilution', '1:7_Control']
            )
            ...
            # Plot significance markers and lines
            p_value_count = 0
    ...
    axes[3, 2].legend(handles=handles, loc='upper left', bbox_to_anchor=(0.3, 0.85), ncol=1)
    ...
    plt.tight_layout()
    plt.savefig('Images/physiochemical_parameters_distributions.png', dpi=config.DPI, bbox_inches='tight')
    plt.clf()
